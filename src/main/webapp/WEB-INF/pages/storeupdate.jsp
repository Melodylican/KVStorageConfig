						...
						<div class="span12">
							...
							<div class="widget-box">
								...
								<div class="widget-content nopadding">
									<form action="<%=request.getContextPath()%>/store/saveupdate" method="post" class="form-horizontal" name="basic_validate" id="basic_validate">
									   ...
									   <div class="control-group">
											...
											<div class="controls">
												<c:choose>
													<c:when test="${not empty storeBean.master}">
														<textarea name="master" rows="10" class="span5" >${storeBean.master }</textarea>
													</c:when>
													<c:otherwise>
														<textarea name="master" rows="10" class="span5" placeholder="请输入json格式的主的配置..."></textarea>
													</c:otherwise>
												</c:choose>	
												<textarea rows="10" disabled="" placeholder="" color="red" class="span5">请按照如下格式进行修改:
												 [{"port":3317,"ip":"192.168.2.44","userName":"root","password":"123456"},{"port":3317,"ip":"192.168.2.44","userName":"root","password":"123456"}]</textarea>
																		
												
												<input name="id" value="${storeBean.id }" type="hidden"/>
											</div>
										</div>
										<div class="control-group">
											...
											<div class="controls">
												<c:choose>
													<c:when test="${not empty storeBean.slave}">
														<textarea name="slave" rows="10" class="span5" >${storeBean.slave }</textarea>
													</c:when>
													<c:otherwise>
														<textarea name="slave" rows="10" class="span5" placeholder="请输入json格式的从的配置..."></textarea>
													</c:otherwise>
												</c:choose>
												<textarea rows="10" disabled="" placeholder="" color="red" class="span5">请按照如下格式进行修改:
